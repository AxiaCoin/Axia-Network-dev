...
console.log('$ axia-dev-build-ts', process.argv.slice(2).join(' '));
...
function buildWebpack () {
  ...
  execSync(`yarn axia-exec-webpack --config ${config} --mode production`);
}
...
async function main () {
  execSync('yarn axia-dev-clean-build');
  ...
  execSync('yarn axia-exec-tsc --emitDeclarationOnly --outdir ../build');
  ...
  if (['js', 'mjs', 'cjs'].some((e) => fs.existsSync(path.join(process.cwd(), `rollup.config.${e}`)))) {
    execSync('yarn axia-exec-rollup --config');
  }
}
