...
console.log('$ axia-ci-ghact-build', process.argv.slice(2).join(' '));
...
function gitBump () {
  ...
  if (argv['skip-beta'] || patch === '0') {
    ...
    execSync('yarn axia-dev-version patch');
  } else {
    ...
    if (tag || patch === '1' || available.includes(currentVersion)) {
      ...
      execSync('yarn axia-dev-version pre');
    } else {
...
skip-checks: true"`);
  ...
  if (doGHRelease) {
    ...
    execSync(`yarn axia-exec-ghrelease --draft ${files} --yes`);
  }
}
